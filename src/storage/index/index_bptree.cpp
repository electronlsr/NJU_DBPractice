...
auto BPTreePage::IsLeaf() const -> bool { return node_type_ == BPTreeNodeType::LEAF; }

auto BPTreePage::IsRoot() const -> bool { return parent_page_id_ == INVALID_PAGE_ID; }

auto BPTreePage::GetSize() const -> int { return size_; }

auto BPTreePage::GetMaxSize() const -> int { return max_size_; }

void BPTreePage::SetSize(int size) { size_ = size; }

auto BPTreePage::GetPageId() const -> page_id_t { return page_id_; }

auto BPTreePage::GetParentPageId() const -> page_id_t { return parent_page_id_; }
...
auto BPTreeLeafPage::GetNextPageId() const -> page_id_t { return next_page_id_; }

void BPTreeLeafPage::SetNextPageId(page_id_t next_page_id) { next_page_id_ = next_page_id; }

auto BPTreeLeafPage::KeyAt(int index) const -> const char * { return GetKeysArray() + index * key_size_; }

auto BPTreeLeafPage::ValueAt(int index) const -> RID { return GetValuesArray()[index]; }
...
auto BPTreeLeafPage::Insert(const Record &key, const RID &value, const RecordSchema *schema) -> int
{
  ...
  if (index < size_) {
    Record current_key(schema, nullptr, KeyAt(index), INVALID_RID);
    if (Record::Compare(key, current_key) == 0) {
      // Duplicate keys not allowed in this lab?
      // "不需要考虑重复值" means we can assume no duplicate keys.
...
auto BPTreeInternalPage::KeyAt(int index) const -> const char * { return GetKeysArray() + index * key_size_; }

auto BPTreeInternalPage::GetKeySize() const -> int { return key_size_; }

auto BPTreeInternalPage::ValueAt(int index) const -> page_id_t { return GetChildrenArray()[index]; }

void BPTreeInternalPage::SetKeyAt(int index, const char *key)
{
  std::memcpy(GetKeysArray() + index * key_size_, key, key_size_);
}
...
{
  // index 0 key is invalid, so start from 1
...
{
  // When merging internal nodes, we need to bring down the key from the parent
...
auto BPTreeIndex::InsertIntoLeaf(const Record &key, const RID &value) -> bool
{
  ...
  // Release guards before updating parent to avoid holding too many frames
  page_guard.Drop();
  new_page_guard.Drop();
...
void BPTreeIndex::InsertIntoParent(page_id_t old_node_id, const Record &key, page_id_t new_node_id)
{
  ...
  // Release guards before recursing
  parent_guard.Drop();
...
auto BPTreeIndex::CoalesceOrRedistribute(page_id_t node_id) -> bool
{
  ...
  if (neighbor_node->GetSize() + node->GetSize() <= node->GetMaxSize()) {
    // Drop guards before calling Coalesce to avoid holding too many frames
    node_guard.Drop();
    parent_guard.Drop();
    neighbor_guard.Drop();
...
auto BPTreeIndex::Coalesce(page_id_t neighbor_node_id, page_id_t node_id, page_id_t parent_id, int index) -> bool
{
  ...
  // Release guards before recursion
  neighbor_guard.Drop();
  node_guard.Drop();
  parent_guard.Drop();
...
void BPTreeIndex::Redistribute(BPTreePage *neighbor_node, BPTreePage *node, int index)
{
  ...
  if (node->IsLeaf()) {
    ...
    if (index == 0) {  // neighbor is to the right
...
auto BPTreeIndex::End() -> std::unique_ptr<IIterator> { return std::make_unique<BPTreeIterator>(this, INVALID_PAGE_ID, 0); }
